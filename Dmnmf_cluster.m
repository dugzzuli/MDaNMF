function [ ac,nmi_value,RI,mAc,mNmi,mRi] = Dmnmf_cluster(X, k, options)
%SYMNMF_CLUSTER
...
for loop=1:1
    for l_i=1:length(lambdas)
        ...
        params.lambda = [lambdas(l_i),lambdas(l_i),lambdas(l_i)];
        params.kmeans=1;
        K = length(unique(gnd));
        params.K=K;
        layers = [169,64,params.K]; %% three layers, the last layer corresponds to the number of communities to detect
        p = numel(layers);
        %% Deep AE NMF
        [U{loop,l_i}, V{loop,l_i},VP{loop,l_i}, dnorm{loop,l_i}, dnormarray{loop,l_i}]=MDaNMF_A(AData, layers, params);
        Vp = VP{loop,l_i}';
        [ac(loop,l_i),nmi_value(loop,l_i),RI(loop,l_i)]=printResult(Vp, gnd', params.K, params.kmeans);
    end
end
mAc=mean(ac);
mNmi=mean(nmi_value);
